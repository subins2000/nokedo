...
$info->execute(array($who));while($r=$info->fetch()){$in=array('name'=>$r['name'],'birth'=>$r['birth'],'join'=>$r['joined'],'gender'=>$r['gender'],'stat'=>$r['stat'],'json'=>$r['json']);$jin=json_decode($in['json'],true);}$img=$jin['img'];$imgs=$jin['imgs'];if($jin['img']==''){$jin['img']="//cdn.nokedo.com/images/guest.png";}if($jin['imgs']==''){$imgs="//cdn.nokedo.com/images/guest.png";}
