...
if($who==$whod){
 $sql=$db->prepare("UPDATE users SET stat=? WHERE `id`=?");
 $sql->execute(array(date('Y-m-d G:i:s'),$who));
 $info=$db->prepare("SELECT * FROM users WHERE id=?");
 $info->execute(array($who));
 while($r=$info->fetch()){
  $in=array('name'=>$r['name'],'birth'=>$r['birth'],'join'=>$r['joined'],'gender'=>$r['gender'],'stat'=>$r['stat'],'json'=>$r['json']);
  $jin=json_decode($in['json'],true);
  
 }
 if($img==''){$jin['img']="//cdn.nokedo.com/images/guest.png";}
 if($jin['imgs']==''){$jin['imgs']="//cdn.nokedo.com/images/guest.png";}
 $img=$jin['img'];$imgs=$jin['imgs'];
}
