<?
if(!isset($_POST)){ser();}
include('config.php');check();function filt($post){$post=str_replace('<','&lt;',$post);$post=str_replace('>','&gt;',$post);$post=preg_replace("/\*\*(.*?)\*\*/",'<b>$1</b>',$post);$post=preg_replace("/\*\/(.*?)\/\*/",'<i>$1</i>',$post); $post=preg_replace("/\[link\((.*?)\)\](.*?)\[\/link\]/",'<a target="_blank" href="$1">$2</a>',$post);$post=preg_replace('@((www|http://)[^ ]+)@', '<a target="_blank" href="//nokedo.com/link.php?url=\1">\1</a>', $post);$post=str_replace("\n","<br/>",$post);$post=preg_replace('@(\#[^ ]+)@','<a href="//class.nokedo.com/explore.php?hash=\1">\1</a>',$post);$post=str_replace("//class.nokedo.com/explore.php?hash=#","//class.nokedo.com/explore.php?hash=%23",$post);return$post;}$t=$_POST['text'];$pr=$_POST['pr'];$i=$_POST['imgs'];if($_POST['type']==''){$_POST['type']='nor';}
if($i==''){
 if($t!='' && $pr!='' && $_POST['type']!=''){ 
  $j='{"time":"'.date("Y-m-d H:i:s").'","pr":"'.$pr.'"}';
  $sql=$db->prepare("INSERT INTO posts(uid,post,ty,json) VALUES(?,?,?,?)");
  $sql->execute(array($who,filt($t),$_POST['type'],$j));
  $sql=$db->prepare("SELECT * FROM posts WHERE uid=? ORDER BY id DESC LIMIT 1");
  $sql->execute(array($who));
  while($r=$sql->fetch()){$id=$r['id'];$time=json_decode($r['json'],true)['time'];}$cont=filt($t);
  $html.="<div class='post ".$_POST['type']."' id='".$id."'><div onclick=\"$('.moptb#$id').toggle();\" class='mopt'></div><div id='$id' class='moptb'><button onclick=\"alert('http://class.nokedo.com/view.php?id=$id');\" class='sb sb-submit'>Link To This Post</button>";
...
echo $html;
 ...
 if($params['ok']!=''){
  ...
  $sql=$db->prepare("INSERT INTO posts(uid,post,ty,json) VALUES(?,?,?,?)");
  $sql->execute(array($who,$cont,$_POST['type'],$j));
  $sql=$db->prepare("SELECT * FROM posts WHERE uid=? ORDER BY id DESC LIMIT 1");
  $sql->execute(array($who));
  while($r=$sql->fetch()){$id=$r['id'];$time=json_decode($r['json'],true)['time'];}
  $html.="<div class='post ".$_POST['type']."' id='".$id."'><div onclick=\"$('.moptb#$id').toggle();\" class='mopt'></div><div id='$id' class='moptb'><button onclick=\"alert('http://class.nokedo.com/view.php?id=$id');\" class='sb sb-submit'>Link To This Post</button>";
