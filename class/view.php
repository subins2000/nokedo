...
include('config.php');
$id=$_GET['id'];
$sql=$db->prepare("SELECT * FROM posts WHERE id=:id AND (uid IN (SELECT fid FROM fds WHERE uid=:who) AND (uid!=:who AND json NOT LIKE '%meo%') OR (json LIKE '%pub%' OR (json LIKE '%fri%' AND uid IN (SELECT fid FROM fds WHERE uid=:who AND fds='1')) OR uid=:who))");
...
<div id="feed" style="margin:5% auto;max-width:600px;">
 <?echo parse($tp);?>
</div>
</body></html>
