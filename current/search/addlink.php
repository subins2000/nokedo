<link href="//nokedo.com/files/css/style2.css" rel="stylesheet" />
...
<input type='submit' class="sb sb-submit" value='Add' name='submit'>
</form>
<?php 
 // Connects to your Database 
 mysql_connect("localhost", "root", "backstreetboys") or die(mysql_error());
 mysql_select_db("db") or die(mysql_error()); 
 //This code runs if the form has been submitted
 if (isset($_POST['submit'])) { 
 //This makes sure they did not leave any fields blank
 if (!mysql_real_escape_string($_POST['title']) | !mysql_real_escape_string($_POST['url']) ) {
 		die('You did not complete all of the required fields');
 	}
 // checks if the title is in use
 	if (!get_magic_quotes_gpc()) {
 		mysql_real_escape_string($_POST['title']) = addslashes(mysql_real_escape_string($_POST['title']));
 	}
...
or die(mysql_error());
 ...
 if ($check2 = 0) {
 		die('Sorry, the Topic/Person <b>'.mysql_real_escape_string($_POST['title']).'</b> is already added. We\'l add a +1 ');
 				}
 // this makes sure both urls entered match
 	// here we encrypt the url and add slashes if needed
 	if (!get_magic_quotes_gpc()) {
 		mysql_real_escape_string($_POST['url']) = addslashes(mysql_real_escape_string($_POST['url']));
 		mysql_real_escape_string($_POST['title']) = addslashes(mysql_real_escape_string($_POST['title']));
 			}
 // now we insert it into the database
 	$insert = "INSERT INTO search (title, url)
 			VALUES ('".mysql_real_escape_string($_POST['title'])."', '".mysql_real_escape_string($_POST['url'])."')";
        echo "Added ".mysql_real_escape_string($_POST['title'])." to Subins <a target='_parent' href='search?q=".mysql_real_escape_string($_POST['title'])."'>Reload</a>";
