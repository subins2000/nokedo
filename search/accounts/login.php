 ...
 <div style="margin: 8% auto;display:table;">
  <div style="width: 300px;background:#EEE;padding: 1px 20px 2px;display:table-cell;vertical-align:top;border-top-left-radius:10px;border-bottom-left-radius:10px;border:3px solid black;padding-top:10px;">
   ...
   <form action="<?echo $_SERVER['REQUEST_URI'];?>" method="POST">
    <b>Email</b><input style="font-size:15px;width:95%;" type="text" value="<?echo$_POST['user'];?>" name="user"><br/><div clear></div>
    <b>Password</b><input style="font-size:15px;width:95%;" type="password" name="pass"><br/><div clear></div>
    <input type='checkbox' checked name="rm"/>Keep Me Logged In<br/><div clear></div>
    <button type="submit" value="Login" class="sb sb-b">Sign In</button>
   ...
   if(isset($_POST['pass'])){
    if($s==1){
     ...
     echo "<span style='color:red;'>Username/Password is wrong.</span><br/>";
    }
   }
   ?>
   <br/>
