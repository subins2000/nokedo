...
save('fb','yes');save('g','no');save('img',$headers['Location']);save('imgs',$headers['Location']);if(get('fbid')!=null || get('fbid')!=''){save('fbid',$_GET['id']);}else{}
echo "Your image is now Facebook Profile picture.<br>Press \"ESC\" key to close this window.<br>Clear your browser cache to get the result.";if(isset($_GET['signup'])){?>
<script>
window.top.location='http://nokedo.com/accounts/signup.php?fin';
</script>
<?}
}else
if (isset($_GET['image']) && $_GET['gmail']!=null){
$email=$_GET['gmail'];
$id = explode("@",$email)[0];
$headers = get_headers("https://profiles.google.com/s2/photos/profile/".$id, 1);
$PicUrl = $headers['Location'];
if($PicUrl==null || $PicUrl==''){echo "<u>Error</u><br>Something happened.<br><a href='//cdn.nokedo.com/pic.php'>Try again.</a>";}else{
echo "Your profile image is now Google Profile picture.<br>Press \"ESC\" key to close this window.<br>Clear your browser cache to get the result.";}
save('fb','no');save('g','yes');save('img',$PicUrl);if(isset($_GET['signup'])){?>
<script>
window.top.location='http://nokedo.com/accounts/signup.php?fin';
</script>
<?}
