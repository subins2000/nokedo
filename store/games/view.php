...
<title><?if($ty!='game'){echo "Game Not Found";}else{echo$t;}?> - OneStore</title>
</head><body>
<div id="content" style="width:60%;">
<?if($ty!='game'){die("<h1>Game Not Found</h1>");}?>
<center style="border-bottom:1px solid #CCC;padding-bottom:5px;"><a href="javascript:window.history.go(-1);" style="float:left;">Go Back</a><a href='//get.nokedo.com'>Home</a><a href="index.php" style="float:right;">All Games</a></center>
<h2 style="display:inline-block;"><?echo$t;?></h2><?if($dv!=''){?><h3 style="display:inline-block;font-weight:normal;">&nbsp;- from <?echo$dv;?></h3><?}?>
<?if($pl!='Downloadable'){echo "<iframe src='$u' frameborder='0' width='100%' height='95%'></iframe><br clear/>";$sql=$db->prepare("UPDATE store SET hits=hits+1 WHERE id=?");$sql->execute(array($_GET['id']));}?>
<div style="display:table-cell;vertical-align: top;<?if($pl!='Downloadable'){?>padding-top: 11px;<?}?>">
<?
if($d!=''){echo '<blockquote style="width: 400px;border: 5px dashed #CCC;padding:8px 10px;margin-left:10px;margin-top:10px;">'.$d.'</blockquote>';}
if($pl!=''){echo "Game Type : <a href='index.php?pl=on'>Online</a> Game<br clear/>";}
if($dv!='' && $pl=='Downloadable'){echo "Devloped By : $dv<br clear/>";echo sts($_GET['id']);}
?>
</div>
<div style="display:table-cell;vertical-align:top;padding-left: 0px;padding-top:10px;">
 <?
 if($dv!='' && $pl!='Downloadable'){
  echo "Devloped By : $dv<br clear/>";
  echo sts($_GET['id']);
  echo "Added on $p by ".un($wu)['name'];
 }elseif($pl=='Downloadable'){?>
 <center style='width:282px;'>
 <button class="sb sb-b" style="display:block;margin:0px auto;padding:10px 25px;font-size:22px;" onclick="$('#dw<?echo$_GET['id'];?>').show();$('#dw<?echo$_GET['id'];?>').find('iframe').attr('src','../get.php?id=<?echo$_GET['id'];?>');">Download</button>
 <div clear></div>
 <b><?echo$h;?></b> Downloads</center>
 <div id="dw<?echo$_GET['id'];?>" style='display:none;'>
  <iframe style="height:0px;width:0px;display:none;" src="about:blank;"/></iframe>
  <center>Download will start in a few moments.<br/>If not <a href="../get.php?id=<?echo$_GET['id'];?>">click here.</a>
  </center>
 </div>
 <?}?>
</div><br/>
